...
import { marketingLinks } from './MarketingNav';

export default function MarketingFooter() {
  return (
    <footer className="border-t border-border bg-fashion-cream-deep">
      <div className="container mx-auto px-4 py-14 md:px-6">
        <div className="grid gap-10 md:grid-cols-[1.5fr_1fr_1fr]">
          <div className="max-w-md">
            <Link to="/" className="mb-4 flex items-center gap-2">
              <span className="flex h-9 w-9 items-center justify-center rounded-full bg-primary font-display text-lg text-primary-foreground">S</span>
              <span className="font-display text-xl">Style<span className="text-secondary">Sense</span></span>
            </Link>
          ...
          <div>
            ...
            <ul className="grid grid-cols-2 gap-x-4 gap-y-2 md:grid-cols-1">
              {marketingLinks.map(link => <li key={link.to}><Link to={link.to} className="text-sm text-muted-foreground hover:text-secondary">{link.label}</Link></li>)}
            </ul>
          </div>
          <div>
            ...
            <ul className="space-y-2 text-sm text-muted-foreground">
              ...
              <li><Link to="/download" className="hover:text-secondary">Download the App</Link></li>
